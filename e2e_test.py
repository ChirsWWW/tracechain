...
SITES = {
    "core": os.environ.get("TC_CORE", "http://127.0.0.1:5059").rstrip("/"),
    "produce": os.environ.get("TC_PRODUCE", "http://127.0.0.1:5060").rstrip("/"),
    "logistics": os.environ.get("TC_LOGISTICS", "http://127.0.0.1:5061").rstrip("/"),
    "retail": os.environ.get("TC_RETAIL", "http://127.0.0.1:5062").rstrip("/"),
    "consume": os.environ.get("TC_CONSUME", "http://127.0.0.1:5063").rstrip("/"),
}
CORE = SITES["core"]
...
st, html = text("/t/" + CODE)
check("验真页提示尚未登记", "尚未登记任何内容" in html)
...
check("验真页含链校验结论", "存证链路完整" in html or "检测到异常" in html)
...
check("恢复备份成功", j.get("ok"), j.get("reason", ""))
