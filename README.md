...
1. **免费实例磁盘是临时的**，核心节点休眠或重新部署会清空数据库。
   ...
   ⚠️ **由此产生一个必须知道的窗口期**：两次冷备之间（最长 5 分钟，`TC_BACKUP_INTERVAL` 可调）
   写入的数据不在快照里，**重启即永久丢失**。表现很有迷惑性——刚在页面上登记成功的码，
   重启后就变成「该追溯码未在本平台赋码」，看起来像业务 bug，其实是数据没进快照。
   因此：跑线上端到端自测、演示前录入数据时，**不要同时触发重新部署**，
   否则会得到一批无法复现的「幽灵失败」。演示数据录完后可手动打一次
   `POST /api/backup/push` 立即落快照，再安心做其他事。
