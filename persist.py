...
def start_worker(store_mod):
    ...
    默认**关闭**（TC_BACKUP_INTERVAL 为 0 或负）：免费层只有 0.1 核 CPU，
    每 5 分钟打包 + 调 GitHub API 会把实例拖慢到 Render 健康检查 5 秒超时，
    进而触发「实例不可用」告警与自动重启——得不偿失。
    需要恢复自动备份时，把环境变量 TC_BACKUP_INTERVAL 设为秒数即可（如 1800）。
    无论是否自动备份，管理台「立即备份」按钮（/api/backup/push）始终可用。
    """
    if not enabled():
        return None
    interval = int(os.environ.get("TC_BACKUP_INTERVAL", "0"))
    if interval <= 0:
        print("[persist] 定时冷备已关闭（手动备份仍可用）", flush=True)
        return None
