...
ITEMS = [
    ("① 完整链路（推荐演示）", "TC260917YEFNP12Y-A2C7",
     "生产 → 流通×3 → 销售核销，含出厂照片凭证"),
    ("② 仅发货未售出", "TC260917PWW56F6F-3BB4",
     "生产 → 流通，流通端留了外箱破损照片"),
    ("③ 空白码（仅赋码）", "TC2609186H06544E-1DA9",
     "尚未在任何端口登记，扫码即可登记"),
]
SITES = [
...
d.text((PAD, PAD + 54), "用手机扫一扫 → 打开公众验真页；五个网址相互独立，数据同源共享",
       font=font(F, 19), fill=SUB)
d.text((PAD, PAD + 82), "登记 / 作业 / 销售 / 验收时间由系统自动生成、不可修改；每端都可附一张现场照片随记录上链",
       font=font(F, 15), fill="#0f6b52")
...
y += 10
d.text((PAD, y), "提示：管理控制台需口令 admin888；五个站点数据实时互通，扫码即可验真。",
